...
type
  ...
  private
    ...
    frequenz,abbruchtime:TLargeInteger;       // Counter reading 1
...
begin
  ...
  // Drawing of the ellipse segments for the 2nd law
  ...
  if radiobutton3.checked then
    can.ellipse(round(mx-e+e2-a2),round(my-b2),round(mx-e+e2+a2+1),
    round(my+b2+1));

  // Calculation of the 1st planet
  ...
  // Determine time-dependent areas using the second law
  if radiobutton2.checked then begin
    if (spinedit3.value=20) then begin
      kep1:=0;
      QueryPerformanceCounter(totalzeit1);       // Counter reading 1
    end;
    if (spinedit3.value>20) then begin
    ...
    if spinedit3.value=round(180+20*(1+epsilon)) then begin
      kep2:=0;
      QueryPerformanceCounter(totalzeit3);       // Counter reading 2
    end;
    if (spinedit3.value>round(180+20*(1+epsilon))) then begin
