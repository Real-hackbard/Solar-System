...
begin
    ...
    linesadd('geographical longitude'#9+edit1.text);
    linesadd('geographical latitude'#9+edit2.text);
    linesadd('Date'#9#9+datetostr(encodedate(j,mon,t)));
    linesadd('World time'#9#9+timetostr(encodetime(h,m,s,0)));

    jd:=juldat(j,mon,t)+h/24+m/24/60+s/24/3600;
    linesadd('Julian date'#9+floattostr(jd));

    szeit:=sternzeit(la,h+m/60+s/3600-1,jd);
    linesadd('sidereal time'#9#9+timetostr(szeit/24));

    Berechnung(jd,'Earth',br,la,0,xastro1,xastro2);
    ...
    linesadd('');
    linesadd('Sun');
    linesadd(format('Right ascension'#9'%.8f h',[zrek]));
    linesadd(format('declination'#9'%.8f°',[zdek]));

    stw:=szeit/12*pi-zrek/12*pi;
    if stw<0 then stw:=stw+2*pi;
    horizont(stw,zdek*pi/180,br,hoehe);
    linesadd(format('Height'#9#9'%.8f°',[hoehe*180/pi]));
    linesadd(format('Pole spacing'#9'%.8f°',[90-hoehe*180/pi]));

    st:=xastro1/15;
    stu:=-23.45*sin(pi*(xastro1+180)/180);
    linesadd('Earth-Sun distance');
    linesadd(#9#9+floattostr(xastro2)+' AE');

    linesadd('Distance between location and Earths center');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          linesadd('Rectification angle Earths center-Sun location');
          linesadd(format(#9#9+'%.9f °',[winkel*180/pi]));
          linesadd('Declination angle Earths center-Sun location');
          linesadd(format(#9#9+'%.9f °',[winkelh*180/pi]));

          linesadd('Distance from location to center of the sun');
          linesadd(#9#9+floattostr(entfernx)+' AE');
          linesadd(format('difference'#9#9+'%.3f km',[(xastro2-entfernx)*149597870700]));

          // Correction of the coordinates for the location
          st:=st+winkel*180/pi/15;
          stu:=stu+winkelh*180/pi;

          linesadd('');
          linesadd('heliocentric coordinates of the location');
          linesadd(format('Right ascension'#9'%.8f h',[st]));
          linesadd(format('declination'#9'%.8f°',[stu]));


          galakt(st/12*pi,stu*pi/180,gl,gb);

          linesadd('galactic coordinates of the location');
          linesadd(format('length'#9#9'%.8f°',[gl]));
          linesadd(format('Width'#9#9'%.8f°',[gb]));

          eklipt(st/12*pi,stu*pi/180,el,eb);

          linesadd('ecliptic coordinates of the location');
          linesadd(format('length'#9#9'%.8f°',[el]));
          linesadd(format('Width'#9#9'%.8f°',[eb]));
        end;
      end;
    end;
    ...
    begin
      linesadd('');
      linesadd('Equator coordinates of Kronos');
      ...
      linesadd('galactic coordinates Kronos');
      linesadd(format('length'#9#9'%.8f°',[gl]));
      linesadd(format('Width'#9#9'%.8f°',[gb]));
    end;
